                ...
                <div x-data="{ openCreate: false }" class="flex flex-col lg:flex-row items-center justify-between gap-4">
                    ...
                    <div x-show="openCreate" class="fixed inset-0 flex items-center justify-center bg-black/50 z-50">
                        ...
                        <div class="bg-white rounded-lg p-6 w-full max-w-lg max-h-[80vh] overflow-y-auto">
                            <h2 class="text-lg font-semibold mb-4">Tambah Kelurahan</h2>

                            <form method="POST" action="{{ route('dashboard.urban-village.store') }}"
                                enctype="multipart/form-data">
                                @csrf

                                <div class="grid grid-cols-1 gap-5">
                                    <x-forms.input name="name" label="Nama Kelurahan"></x-forms.input>
                                </div>
                    ...
                    {{-- Form Filter & Search --}}
                    <form method="GET" action="{{ route('dashboard.urban-village.index') }}"
                        ...
                        <div class="w-full lg:w-80">
                            <x-forms.input type="text" name="search" placeholder="Cari kelurahan..."
            ...
            <table class="min-w-full text-sm">
                <thead class="bg-[#486284] text-gray-50">
                    <tr>
                        ...
                        <th class="p-2 font-normal text-left border-r border-gray-600">Kelurahan</th>
                ...
                <tbody class="divide-y divide-gray-300">
                    @forelse($urbanVillages as $urbanVillage)
                        <tr class="hover:bg-blue-50">
                            <td class="p-2 text-center border-r border-gray-300">
                                {{ ($urbanVillages->currentPage() - 1) * $urbanVillages->perPage() + $loop->iteration }}
                            </td>
                            <td class="p-2 border-r border-gray-300 whitespace-nowrap">{{ $urbanVillage->name }}</td>
                            <td class="p-2 text-center border-r border-gray-300 whitespace-nowrap">
                                {{ \Carbon\Carbon::parse($urbanVillage->created_at)->format('d/m/Y H:i') }}
                            </td>
                            <td class="p-2 whitespace-nowrap">
                                <div class="flex justify-center items-center gap-2">
                                    <div x-data="{ openEdit: false }">
                                        ...
                                        <div x-show="openEdit"
                                            ...
                                            <div
                                                ...
                                                <h2 class="text-lg font-semibold mb-4">Edit Kelurahan</h2>

                                                <form method="POST"
                                                    action="{{ route('dashboard.urban-village.update', $urbanVillage->slug) }}">
                                                    @csrf
                                                    @method('PUT')

                                                    <div class="grid grid-cols-1 gap-5">
                                                        <x-forms.input name="name" label="Nama Kelurahan"
                                                            :value="old('name', $urbanVillage->name)"></x-forms.input>
                                                    </div>
                                    ...
                                    <form action="{{ route('dashboard.urban-village.destroy', $urbanVillage->slug) }}"
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="p-4 text-center text-gray-500">Tidak ada data kelurahan</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
